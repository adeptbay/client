...
import { activeCategories, countInCategory, recentTools, topTools, totalLive } from '@core/registry';
      ...
      <section className="border-b border-line bg-gradient-to-b from-brand-soft/60 to-surface">
        <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-fg-muted sm:text-lg">
            Fast, free utilities for developers, students and businesses. Most of them run entirely
            in your browser — so your files are never uploaded, and there is no sign-up, no quota and
            no watermark.
          </p>
      ...
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        ...
        <ul className="grid gap-4 sm:grid-cols-3">
          {site.promises.map((promise, i) => (
            <Card as="li" key={promise.title} className="px-5 py-4">
              ...
              <h2 className="mt-2.5 text-sm font-semibold text-fg">{promise.title}</h2>
        ...
        <section className="mt-16">
          <div className="flex items-baseline justify-between gap-4">
            <h2 className="text-xl font-semibold tracking-tight text-fg">Browse by division</h2>
            ...
            {categories.map((category) => (
              <li key={category.slug}>
                ...
                >
                  ...
                  <p className="mt-2 text-[13px] leading-relaxed text-fg-muted">{category.tagline}</p>
                </Link>
              </li>
            ))}
          </ul>
        ...
        {/* ── Most used ────────────────────────────────────────── */}
        <section className="mt-16">
          <h2 className="text-xl font-semibold tracking-tight text-fg">Start here</h2>
        ...
        {/* ── Recently added ───────────────────────────────────── */}
        <section className="mt-16">
          <h2 className="text-xl font-semibold tracking-tight text-fg">Recently added</h2>
          <ul className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {recent.map((tool) => (
              <li key={`${tool.category}/${tool.slug}`}>
                <ToolCard tool={tool} showCategory />
        ...
        <section className="mt-16 rounded-2xl border border-line bg-sunken px-6 py-8 sm:px-10 sm:py-10">
          ...
          <div className="prose-bay mt-4 max-w-3xl">
            <p>
              Most online tools work by uploading your file to a server, processing it there, and
              sending the result back. That means a copy of your document existed on someone else’s
              machine, was written to their disk, and passed through their logs.
            </p>
            <p>
              The tools here marked <strong>Offline</strong> do the work in your own browser using
              WebAssembly and the platform’s built-in APIs. There is no upload step, because there is
              nowhere to upload to. You can verify it: load a tool page, disconnect from the
              internet, and it keeps working.
            </p>
            <p>
              Some jobs genuinely need a server — large video transcoding, for instance. Those tools
              say so on the page, delete your file within two hours, and never log its contents.{' '}
              <Link href="/privacy">The privacy policy</Link> states exactly which is which.
            </p>
          </div>
        </section>
      </div>
